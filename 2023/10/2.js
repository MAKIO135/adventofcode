...
const isPointInPolygon = (polygonPts, pt) => {
    let c = false
    for (let i = 0, j = polygonPts.length-1; i < polygonPts.length; j = i++) {
        if (
            ((polygonPts[i].y > pt.y) != (polygonPts[j].y > pt.y)) && 
            (pt.x < (polygonPts[j].x-polygonPts[i].x) * (pt.y-polygonPts[i].y) / (polygonPts[j].y-polygonPts[i].y) + polygonPts[i].x)
        ) c = !c
    }
    return c
}
...
fs.readFile('./input', 'utf8', (err, input) => {
    ...
    let insideLoop = input.reduce((acc, l, y) => acc + l.reduce((acc2, c, x) => {
        let isPipe = positions.some(p => p.x === x && p.y === y)
        if(!isPipe) {
            if(isPointInPolygon(positions, {x, y})) {
                input[y][x] = 'I'
                acc2 ++
            }
            else input[y][x] = '.'
