...
fs.readFile('./input', 'utf8', (err, input) => {
    ...
    const compute = (start, dir) => {
        ...
        return map.flat().filter(d => d === '#').length
    }

    const energized = [
        ...array(ny).map(y => compute({x:0, y}, {x:1, y:0})),
        ...array(ny).map(y => compute({x:nx-1, y}, {x:-1, y:0})),
        ...array(nx).map(x => compute({x, y:0}, {x:0, y:1})),
        ...array(nx).map(x => compute({x, y:ny-1}, {x:0, y:-1})),
    ]
    log(Math.max(...energized))
})